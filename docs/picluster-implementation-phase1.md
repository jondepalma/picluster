...
**Tasks:**
- [ ] Flash Raspberry Pi OS Lite (64-bit) to all SD cards
- [ ] Attach 1TB NVMe SSD to pi-control (M.2 HAT)
- [ ] Boot all nodes and complete initial setup
...
- [ ] Update all systems: `sudo apt update && sudo apt upgrade -y`
- [ ] Mount and format NVMe on pi-control:
  ```bash
  # On pi-control
...
**Tasks:**
- [ ] Assign static IP: 10.0.0.5
- [ ] Enable NFS service (Control Panel → Network & File Services → NFS)
- [ ] Create shared folders:
  - `/share/kubernetes-pv` (500GB) - Persistent volumes
  - `/share/kubernetes-backups` (100GB) - Backups
- [ ] Configure NFS permissions:
  - Allow 10.0.0.0/24 network
  - Read/Write access
  - Enable "No root squash"
- [ ] Test NFS from pi-control:
  ```bash
...
# Should show EXTERNAL-IP in 10.0.0.100-120 range
```
